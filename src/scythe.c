...
void usage(int status) {
  ...
  -g, --gzip-output     output gzipped trimmed sequences file (default: off)\n\
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		int option_index = 0;
		optc = getopt_long(argc, argv, "gdtfp:a:o:q:m:o:n:M:", long_options, &option_index);
  ...
  */
	while ((l = kseq_read(seq)) >= 0) {
		...
		if (!gzip_output){
			...
			write_fastq_gzip(output_gzip, seq, add_tag, shift, min_keep);
		}
